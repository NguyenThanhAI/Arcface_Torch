...
class FaceTracker:
    ...
    def match_reid_iou_tentative(self, face_detections):
        ...
        for r, c in zip(row_ind, col_ind):
            if iou_matrix[r, c] >= self.reid_iou_threshold and self.tentative_tracks[r].class_id == face_detections[c].class_id:
    ...
    def match_reid_iou_active(self, face_detections):
        ...
        for r, c in zip(row_ind, col_ind):
            if iou_matrix[r, c] >= self.reid_iou_threshold and self.active_tracks[r].class_id == face_detections[c].class_id:
                matched_track_indx.append(r)
                matched_detection_indx.append(c)
    ...
    def match_reid_iou_inactive(self, face_detections):
        ...
        for r, c in zip(row_ind, col_ind):
            if iou_matrix[r, c] >= self.reid_iou_threshold and self.inactive_tracks[r].class_id == face_detections[c].class_id: # Huhu đáng ra phải là self.inactive_tracks[r].class_id nhưng viết nhầm thành self.active_tracks[r].class_id
